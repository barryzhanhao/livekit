...
func NewWebRTCConfig(conf *config.Config) (*WebRTCConfig, error) {
	...
	// NAT/K8s: decouple the externally advertised media IP (advertise_ip) from the
	// internal node routing IP (node_ip). rtcconfig reuses node_ip for both ICE
	// candidates and inter-node routing; here we redirect only the ICE-facing role to
	// advertise_ip, leaving node_ip untouched for inter-node PSRPC routing
	// (see routing.NewLocalNode). rtcConf is a value copy, so conf.RTC.NodeIP is unchanged.
	if !conf.RTC.AdvertiseIP.IsEmpty() {
		rtcConf.RTCConfig.NodeIP = conf.RTC.AdvertiseIP
		rtcConf.RTCConfig.NodeIPAutoGenerated = false
		rtcConf.RTCConfig.UseExternalIP = false
	}
